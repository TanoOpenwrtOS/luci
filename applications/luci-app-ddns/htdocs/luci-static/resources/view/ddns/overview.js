...
return view.extend({
	...
	*/
	...
	render: function(data) {
		...
		o = s.taboption('global', form.Value, 'cacert', _('CA certificates path'));
		...
		s.handleAdd = function(ev) {
			...
			m2.render().then(L.bind(function(nodes) {
				ui.showModal(_('Add new service...'), [
					nodes,
					E('div', { 'class': 'right' }, [
						E('button', {
							'class': 'btn',
				...
				if (s.service_available && s.service_supported) {
					...
					if (env['has_ssl']) {
						...
						o = s.taboption('basic', form.Value, 'cacert',
							...
							_("Directory or path/file or <b>IGNORE</b> " +
							  "to run HTTPS without verification of server certificates (insecure)"));
						o.modalonly = true;
					...
					o = s.taboption('advanced', form.Value, 'ip_url',
						...
						+ '</li><li>' +
						String.format('%s%s', _('Example for IPv6'), ': http://checkipv6.dyndns.com') + '</li></ul>');
					...
					o = s.taboption("timer", form.Value, "retry_count",
						...
						_("On Error the script will stop execution after given number of retries. " +
