...
var UITextfield = UIElement.extend(/** @lends LuCI.ui.Textfield.prototype */ {
	...
	render: function() {
		...
		var inputEl = E('input', {
			...
			'data-type': this.options.datatype ? this.options.datatype : '',
...
var UI = baseclass.extend(/** @lends LuCI.ui.prototype */ {
			...
			if (checked) {
				...
				var call = function(r, data, duration) {
					...
					var rollbackTimerId = window.setTimeout(function rollbackConfirm() {
						...
						}).then(call, call.bind(null, { status: 0 }, null, 0)).catch(function(e) {
	 ...
	 */
	instantiateView: function(path, version) {
		var className = 'view.%s'.format(path.replace(/\//g, '.'));

		return L.require(className, null, version).then(function(view) {
