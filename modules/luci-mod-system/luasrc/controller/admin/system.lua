...
function index()
	...
	entry({"admin", "system", "system"}, view("system/system"), _("General Settings"), 1)
	entry({"admin", "system", "time"}, view("system/time"), _("Time"), 1)

	entry({"admin", "system", "admin"}, firstchild(), _("Administration"), 2)
	entry({"admin", "system", "admin", "password"}, view("system/password"), _("Device Password"), 1)
